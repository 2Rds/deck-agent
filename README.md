...
10. **Inngest event name `deck/uploaded` and `{deckId}` payload shape.** Implementation detail of deviation #1 (Inngest replacing Cloudflare Worker). Inngest v4 dropped the `EventSchemas` class that older versions used for compile-time event typing, so the registry lives as typed wrapper functions in `lib/inngest/client.ts` (`sendDeckUploaded(deckId)`, `sendDeckFailed(deckId, reason)`). Callers use the wrappers; direct `getInngest().send()` calls are unchecked by design and should be avoided.
